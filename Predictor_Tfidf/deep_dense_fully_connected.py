"""Use a deep ANN to find the probability of occurrence of diseases"""
import tflearn
import numpy as np
import pandas as pd
...
name = 'DFC_n_epoch_' + str(n_epoch) + '_batch_size_' + str(batch_size) \
...
for c, d in enumerate(uniq_diag):
    ...
    with tf.Graph().as_default():
        ...
        joblib.dump(sc, 'Saved_Models/Deep_Fully_Connected_n_epochs_{}/standard.pkl'.format(n_epoch))
        ...
        dense3 = tflearn.fully_connected(dropout2, 128, activation='linear', name='dense3')
        dropout3 = tflearn.dropout(dense3, 0.8)
        dense4 = tflearn.fully_connected(dropout3, 128, activation='linear', name='dense4')
        dropout4 = tflearn.dropout(dense4, 0.8)
        output = tflearn.fully_connected(dropout4, 2, activation='softmax', name='output')
        ...
        model.fit(X_train_sd, Y_train, n_epoch=n_epoch, show_metric=True, snapshot_epoch=True,
        ...
        model.save('Saved_Models/Deep_Fully_Connected_n_epochs_{0}/deep_dense_fully_connected_dropout_5645_{1}.tfl'
                   .format(n_epoch, d))
...
print('ROC Plot saved at ../Results_tfidf/Deep_Dense_fully_connected/Plots/ROC_' + name)
print("--------------------Training Done!!!--------------------")
...
plt.savefig('../Results_tfidf/Deep_Dense_fully_connected/Plots/ROC_' + name + '.png')
