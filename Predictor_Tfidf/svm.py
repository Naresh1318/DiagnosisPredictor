"""Performs SVM to find the probability of occurrence of diseases"""
import numpy as np
import pandas as pd
...
name = 'SVM_kernel_' + kernel + '_gamma_' + str(gamma) + '_C_' + str(C) + '_size_' + str(size)  # name of ROC Plot
...
for c, d in enumerate(uniq_diag):
    ...
    joblib.dump(sc, 'Saved_Models/SVM_tfidf/standard.pkl')
    ...
    joblib.dump(svm, 'Saved_Models/SVM_tfidf/svm_k_{0}_g_{1}_C_{2}_{3}.pkl'.format(kernel, gamma, C, d))

    Y_pred_lr = svm.predict(X_test_sd)
    ...
    Y_prob = svm.predict_proba(X_test_sd)
...
print('ROC Plot saved at ../Results_tfidf/SVM/Plots/ROC_' + name)
print("--------------------Training Done!!!--------------------")
...
plt.savefig('../Results_tfidf/SVM/Plots/ROC_' + name + '.png')
