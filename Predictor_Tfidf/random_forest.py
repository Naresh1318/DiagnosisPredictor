"""Performs Random Forest to find the probability of occurrence of diseases"""
import numpy as np
import pandas as pd
...
for c, d in enumerate(uniq_diag):
    ...
    joblib.dump(sc, 'Saved_Models/Random_Forest_tfidf/standard.pkl')
    ...
    joblib.dump(forest, 'Saved_Models/Random_Forest_tfidf/rf_ne_{0}_{1}.pkl'.format(n_estimators, d))
...
print('ROC Plot saved at ../Results_tfidf/Random_Forest/Plots/ROC_' + name)
print("--------------------Training Done!!!--------------------")
...
plt.savefig('../Results_tfidf/Random_Forest/Plots/ROC_' + name + '.png')
