"""Use an LSTM to find the probability of occurrence of diseases"""
...
for c, d in enumerate(uniq_diag[:8]):
    ...
    with tf.Graph().as_default():
        ...
        joblib.dump(sc, 'Saved_Models/LSTM_n_epochs_{}/standard.pkl'.format(n_epoch))
        ...
        model.fit(X_train_sd, Y_train, n_epoch=n_epoch, show_metric=True, snapshot_epoch=True, snapshot_step=500,
                  run_id='model_and_weights', validation_set=(X_test_sd, Y_test), batch_size=batch_size)
        ...
        model.save('Saved_Models/LSTM_n_epochs_{0}/LSTM_{1}.tfl'.format(n_epoch, d))
...
print('ROC Plot saved at ../Results_tfidf/LSTM/Plots/ROC_' + name)
print("--------------------Training Done!!!--------------------")
...
plt.savefig('../Results_tfidf/LSTM/Plots/ROC_' + name + '.png')
