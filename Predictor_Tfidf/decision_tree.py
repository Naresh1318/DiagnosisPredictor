"""Performs decision tree to find the probability of occurrence of diseases"""
import numpy as np
import pandas as pd
...
name = 'DT_' + '_criterion_' + criterion + '_max_depth_' + \
       str(max_depth) + '_size_' + str(size)  # name of ROC Plot
...
for c, d in enumerate(uniq_diag):
    ...
    joblib.dump(sc, 'Saved_Models/Decision_Tree_tfidf/standard.pkl')
    ...
    joblib.dump(dr, 'Saved_Models/Decision_Tree_tfidf/Decision_Tree_{}.pkl'.format(d))
    ...
    Y_prob = dr.predict_proba(X_test_sd)
...
print('ROC Plot saved at ../Results_tfidf/Decision_Tree/Plots/ROC_' + name)
print("--------------------Training Done!!!--------------------")
...
plt.savefig('../Results_tfidf/Decision_Tree/Plots/ROC_' + name + '.png')
