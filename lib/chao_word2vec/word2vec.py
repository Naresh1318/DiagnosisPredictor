...
import sys
import os
import heapq #小顶堆排序算法
from timeit import default_timer #计时工具timeit模块
from copy import deepcopy #深拷贝 创建一个新的组合对象，同时递归地拷贝所有子对象，新的组合对象与原对象没有任何关联.
from collections import defaultdict
import threading #并发模块
import itertools #处理迭代功能的函数，它们的返回值不是list，而是迭代对象，只有用for循环迭代的时候才真正计算.
...
class Word2Vec(utils.SaveLoad):
    ...
    def __init__(
            ...
            pre='time_win_pre.csv', suf='time_win_suf.csv'):
        """
    ...
    def _do_train_job(self, sentences, alpha, inits):
        ...
        if self.sg:
            ...
            for i in range(len(win)):
                results = map(int, win.ix[i,0].split())
                self.array_suf.append(results)
            print self.array_pre
            print self.array_suf
            '''
