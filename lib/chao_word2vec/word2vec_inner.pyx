...
def train_batch_cbow(model, sentences, alpha, _work, _neu1):
    ...
    cdef np.uint32_t indexes[MAX_SENTENCE_LEN]
    #cdef np.uint32_t reduced_windows[MAX_SENTENCE_LEN]
    #cdef np.uint32_t reduced_windows_pre[MAX_SENTENCE_LEN][MAX_SENTENCE_LEN]
    #cdef np.uint32_t reduced_windows_suf[MAX_SENTENCE_LEN][MAX_SENTENCE_LEN]
    #cdef int sentence_idx[MAX_SENTENCE_LEN + 1]
    #cdef int window = 5
	
	#Modified by Chao
	#define the window_pre and window_suf parameters
	...
	# precompute "reduced window" offsets in a single randint() call
    z=0
    for i, item in enumerate(model.random.randint(0, window[z], effective_words)):
        reduced_windows[i] = item
        z+=1
    
    for i in range(len(window_pre)):
        for j in range(len(window_pre[i])):
            reduced_windows_pre[i][j] = model.random.randint(0, window_pre[i][j])
    for i in range(len(window_suf)):
        for j in range(len(window_suf[i])):
            reduced_windows_suf[i][j] = model.random.randint(0, window_suf[i][j])
    '''
	#Modified by Chao
    ...
    with nogil:
        ...
        for sent_idx in range(effective_sentences):
            ...
            idx_end = sentence_idx[sent_idx + 1]
            for i in range(idx_start, idx_end):
                #j = i - window_pre[sent_idx][i] + reduced_windows_pre[sent_idx][i]
                j = i - window_pre[sent_idx][i]
                if j < idx_start:
                    j = idx_start
                #k = i + window_suf[sent_idx][i] + 1 - reduced_windows_suf[sent_idx][i]
